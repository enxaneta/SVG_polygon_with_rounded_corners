let r = 10; //roundness
// set the viewBox for the SVG elements
setViewBox(poly);

function setViewBox(thePath) {
  let BB = thePath.getBBox();
  let viewBx = `${BB.x - 10} ${BB.y - 10} ${BB.width + 20} ${BB.height + 20}`;
  a.setAttributeNS(null, "viewBox", viewBx);

  b.setAttributeNS(null, "viewBox", viewBx);
}

console.log(getPoints(poly));
...
let d = polygonWithRoundedCorners(poly, r);

roundedCorners.setAttributeNS(null, "d", d);
output.value = d;
